...
static void quasi_static(const signal<3> & sig, const device & d, int N = 400) {
    int step = sig.N_t / N;
    ...
    std::vector<current> i_quasi = curve(d.p, points);
    // save for plotting
    arma::mat CSV (N, 2);
    for (int j = 0; j < N; ++j) {
        CSV(j, 0) = c::dt * step * j;
    ...
    CSV.save(save_folder() + "/" + d.name + "/I_quasi.csv", arma::csv_ascii);
}
